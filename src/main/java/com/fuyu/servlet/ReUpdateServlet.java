...
public class ReUpdateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String  search_sex = request.getParameter("select_sex");
        if (search_sex.trim().equals("男")){
            search_sex = "m";
        }else {
            search_sex ="f";
        }

        String search_birthday = request.getParameter("birthday");
        if(search_name.trim().equals("")&&search_sex.trim().equals("")&&search_birthday.trim().equals("")){
            request.getRequestDispatcher("/main.jsp").forward(request, response);
            return;
        }

        User user = new User();
        user.setName(search_name);


        user.setSex(search_sex);
        user.setBirth(search_birthday);
        user.setNote("");
        user.setId(0);
        user.setPhone("");
        ...
        try {
            ResultSet resultSet = userService.searchBySelect(user);
